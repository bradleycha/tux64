...
/*----------------------------------------------------------------------------*/
/* mkrom/src/tux64-mkrom/kernel.c - Implementations for kernel ELF parser.    */
/*----------------------------------------------------------------------------*/
...
#define TUX64_MKROM_KERNEL_ENDIAN_FORMAT \
   TUX64_ENDIAN_FORMAT_BIG

static struct Tux64MkromKernelParseResult
tux64_mkrom_kernel_parse_elf_header_common(
   const union Tux64ElfHeader * elf_header
) {
   struct Tux64MkromKernelParseResult result;
   Tux64UInt16 type;
   Tux64UInt16 machine;
   Tux64UInt32 version;

   type = tux64_endian_convert_uint16(elf_header->elf32.e_type, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   machine = tux64_endian_convert_uint16(elf_header->elf32.e_machine, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   version = tux64_endian_convert_uint32(elf_header->elf32.e_version, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   ...
   if (type != TUX64_ELF_HEADER_TYPE_EXEC) {
      result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_INVALID_TYPE;
   ...
   if (machine != TUX64_ELF_HEADER_MACHINE_MIPS) {
      result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_INVALID_MACHINE;
   ...
   if (version != TUX64_ELF_HEADER_VERSION_1) {
      result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_BAD_VERSION;
      result.payload.bad_version.version = version;
      return result;
   }

   result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_OK;
   return result;
}

static Tux64Boolean
tux64_mkrom_kernel_program_headers_present(
...
static struct Tux64MkromKernelParseResult
tux64_mkrom_kernel_parse_program_headers_32(
   ...
   while (phnum != TUX64_LITERAL_UINT16(0u)) {
      ...
      segment_type = tux64_endian_convert_uint32(program_header.p_type, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      ...
      if (main_segment_found == TUX64_BOOLEAN_TRUE) {
         result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_MAIN_SEGMENT_DUPLICATE;
         return result;
      }
      main_segment_found = TUX64_BOOLEAN_TRUE;

      /* load the relevant information about the segment */
      segment_file_offset = tux64_endian_convert_uint32(program_header.p_offset, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      image_bytes = tux64_endian_convert_uint32(program_header.p_filesz, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      image_memory = tux64_endian_convert_uint32(program_header.p_memsz, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      addr_load = tux64_endian_convert_uint32(program_header.p_vaddr, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      alignment = tux64_endian_convert_uint32(program_header.p_align, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);

      /* verify the segment data offset/length */
      if (segment_file_offset + image_bytes > elf_bytes) {
         result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_CORRUPT_IMAGE;
         return result;
      }

      /* compute the image data address */
      image_data = &elf_data[segment_file_offset];
   }

   /* done like this so we can both detect duplicate and missing main segments. */
   if (main_segment_found == TUX64_BOOLEAN_FALSE) {
      result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_MAIN_SEGMENT_MISSING;
      return result;
   }

   /* great success! */
   result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_OK;
   result.payload.ok.image.data = image_data;
   result.payload.ok.image.bytes = image_bytes;
   result.payload.ok.image.memory = image_memory;
...
tux64_mkrom_kernel_parse_32(
   ...
   result = tux64_mkrom_kernel_parse_elf_header_common((const union Tux64ElfHeader *)&elf_header);
   if (result.status != TUX64_MKROM_KERNEL_PARSE_STATUS_OK) {
      return result;
   }

   ehsize = tux64_endian_convert_uint16(elf_header.e_ehsize, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   phsize = tux64_endian_convert_uint16(elf_header.e_phentsize, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   ...
   addr_entry = tux64_endian_convert_uint32(elf_header.e_entry, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   phoff = tux64_endian_convert_uint32(elf_header.e_phoff, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   phnum = tux64_endian_convert_uint16(elf_header.e_phnum, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);

   /* verify the program headers don't run off the end of the file */
   if (tux64_mkrom_kernel_program_headers_present(
      elf_bytes,
      phoff,
      phnum,
   ...
   return tux64_mkrom_kernel_parse_program_headers_32(
      elf_data,
      elf_bytes,
      addr_entry,
      phoff,
...
static struct Tux64MkromKernelParseResult
tux64_mkrom_kernel_parse_program_headers_64(
   const Tux64UInt8 * elf_data,
   Tux64UInt32 elf_bytes,
   Tux64UInt32 addr_entry,
   Tux64UInt32 phoff,
   ...
   while (phnum != TUX64_LITERAL_UINT16(0u)) {
      ...
      segment_file_offset = tux64_endian_convert_uint64(program_header.p_offset, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      image_bytes = tux64_endian_convert_uint64(program_header.p_filesz, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      image_memory = tux64_endian_convert_uint64(program_header.p_memsz, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      addr_load = tux64_endian_convert_uint64(program_header.p_vaddr, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
      alignment = tux64_endian_convert_uint64(program_header.p_align, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   ...
   result.payload.ok.image.memory = (Tux64UInt32)image_memory;
...
tux64_mkrom_kernel_parse_64(
   ...
   addr_entry = (Tux64UInt32)tux64_endian_convert_uint64(elf_header.e_entry, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);

   /* however, it's NOT fine to truncate here, as it could be due to either file limitations or bad data. */
   phoff64 = tux64_endian_convert_uint64(elf_header.e_phoff, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   phnum = tux64_endian_convert_uint16(elf_header.e_phnum, TUX64_MKROM_KERNEL_ENDIAN_FORMAT);
   ...
   return tux64_mkrom_kernel_parse_program_headers_64(
      elf_data,
      elf_bytes,
      addr_entry,
      phoff,
...
static Tux64Boolean
tux64_mkrom_kernel_validate_alignment(
   Tux64UInt32 alignment
) {
...
tux64_mkrom_kernel_parse(
   ...
   const struct Tux64MkromKernel * kernel;
   ...
   if (elf_data[TUX64_ELF_HEADER_IDENT_INDEX_DATA] != TUX64_ELF_HEADER_IDENT_DATA_ENDIAN_BIG) {
      result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_INVALID_ENDIANESS;
      return result;
   }
   ...
   switch (elf_data[TUX64_ELF_HEADER_IDENT_INDEX_CLASS]) {
      case TUX64_ELF_HEADER_IDENT_CLASS_32:
         result = tux64_mkrom_kernel_parse_32(elf_data, elf_bytes);
         break;

      case TUX64_ELF_HEADER_IDENT_CLASS_64:
         result = tux64_mkrom_kernel_parse_64(elf_data, elf_bytes);
         break;
   ...
   if (kernel->image.memory < kernel->image.bytes) {
      result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_CORRUPT_IMAGE;
      return result;
   }
   if (tux64_mkrom_kernel_validate_alignment(kernel->alignment) == TUX64_BOOLEAN_FALSE) {
      result.status = TUX64_MKROM_KERNEL_PARSE_STATUS_CORRUPT_IMAGE;
      return result;
   }
